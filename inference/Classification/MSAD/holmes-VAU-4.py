...
class_keywords = ['Assault', 'Explosion', 'Fighting', 'Fire', 'Object_falling', 
                 'People_falling', 'Robbery', 'Shooting', 'Traffic_accident', 
                 'Vandalism', 'Water_incident', 'Normal']

test_data_path = build_data_path('MSAD', 'msad_test_anno.json')  
video_path_prefix = build_data_path('MSAD', 'other_datasets', 'msad_videos')    
output_file = build_output_path('inference', 'Classification', 'MSAD', 'holmes-vau-4.txt')
...
for item in test_data:
    ...
    try:
        ...
        if "traffic accident" in pred.lower():
            predicted_class = "Traffic_accident"
        else:
